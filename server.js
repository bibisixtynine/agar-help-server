...
// 20 mai 2022
// supression variables inutilisées
// ajout du mode d'éxecution (localDebugVersion)

let localDebugVersion = true;
...
app.get("/", function (request, response) {
  ...
  response.send(
    style() + `<body>` + div1() + div2(nbClientConnected) + script() + `</body>`
  );
});
...
if (localDebugVersion) {
  expressHttpServer.listen(3000, "0.0.0.0");
  zutils.log("🖖 express http LOCAL server started 🤩");
} else {
  expressHttpServer.listen(3000);

  zutils.log("🖖 express http GLITCH server started 🤩");
}
//
//
// HTTP EXPRESS SERVER
